 ...
 *
 * Updates of P-M-S calculation
 * Copyright (C) 2022-2025 congatec GmbH
 * Author: Lukas Posadka <lukas.posadka@congatec.com>
 *
				...
				const struct drm_display_mode *mode)
{
	...
	static uint64_t bit_freq_prev;
	static uint64_t pix_freq_prev;
	unsigned int inFreq = dsim->pref_clk * 1000;
	...
	if ((pix_freq == pix_freq_prev)
		&& (bit_freq == bit_freq_prev)
		) {
		return 0;
	} else {
		pix_freq_prev = pix_freq;
		bit_freq_prev = bit_freq;
	}

	dev_info(dsim->dev, "bitfreq is: %llu\n", bit_freq);
	dev_info(dsim->dev, "pixfreq is: %llu\n", pix_freq);
	...
	dev_info(dsim->dev, "SCAN PMS: %u %u %u %u 0x%x\n", dsim->bit_clk, dsim->p, dsim->m, dsim->s, dsim->pms);
