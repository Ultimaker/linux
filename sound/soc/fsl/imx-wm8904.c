				     ...
				     struct snd_pcm_hw_params *params)
{
	...
	if (!data->is_codec_master) {
		...
		ret = snd_soc_dai_set_sysclk(cpu_dai, WM8904_CLK_MCLK, data->clk_frequency, SND_SOC_CLOCK_OUT);
		if (ret) {
			dev_err(dev, "failed to set cpu sysclk: %d\n", ret);
			return ret;
		}

		ret = snd_soc_dai_set_sysclk(codec_dai, WM8904_CLK_MCLK, data->clk_frequency, SND_SOC_CLOCK_OUT);
		if (ret) {
			dev_err(dev, "failed to set cpu sysclk: %d\n", ret);
			return ret;
		}
